...
export class OptimizationService {
  ...
  static async startOptimization(analysisId: string): Promise<OptimizationJobResponse> {
    try {
      ...
      const response = await analysisApiClient.post<OptimizationJobResponse>(
        `/analyze/${analysisId}/optimize`
      );
      ...
      if (error.response) {
        ...
        const errorMessage =
          data?.message || data?.error || `Failed to start optimization (${status})`;
  ...
  static async getJobStatus(jobId: string): Promise<JobStatusResponse> {
    try {
      ...
      try {
        ...
        if (status && status !== 404 && status !== 403) throw e;
