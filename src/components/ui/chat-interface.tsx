...
/**
 * Link badge component for displaying clickable links (visual only)
 */
              ...
              {message.type === "image" && (
                <div className="relative min-h-32">
                  ...
                  <img
                    src={message.content}
                    alt="Chat"
                    className={`h-auto max-h-full max-w-48 rounded object-cover ${
                      !imageLoaded ? "hidden" : ""
                    }`}
        ...
        <AnimatePresence mode="wait">
          {shouldShowAvatar ? (
            <motion.img
              ...
              src={person.avatar}
              alt={person.name}
              className="h-8 w-8 flex-shrink-0 rounded-full border-[1.5px] border-white object-cover"
            ...
            <div className="h-8 w-8 flex-shrink-0" key="spacer" />
          )}
        </AnimatePresence>
...
const ChatComponent: React.FC<ChatComponentProps> = ({
  ...
  return (
    ...
    >
      <div
        className="pointer-events-none absolute left-0 right-0 top-0 h-32 rounded-t-lg"
        style={{ background: gradientBackground }}
      />

      <div
        ref={containerRef}
        className="scrollbar-hide h-full overflow-y-scroll p-8"
        style={{
          scrollbarWidth: "none",
