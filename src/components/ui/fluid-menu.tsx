  ...
  return (
    <button
      className={`relative block w-full h-12 text-center group
        ${disabled ? "text-gray-400 dark:text-gray-500 cursor-not-allowed" : "text-foreground"}
    ...
    <div className="relative w-[48px]" data-expanded={isExpanded}>
      ...
      <div className="relative">
        ...
        {childrenArray.slice(1).map((child, index) => (
          <div 
            key={index} 
            className="absolute top-0 left-0 w-12 h-12 bg-card/90 backdrop-blur-xl border border-border will-change-transform rounded-full shadow-md hover:shadow-lg"
            style={{
