...
export const useUnifiedAnalysisPolling = (config: Partial<PollingConfig> = {}) => {
  ...
  const pollStatus = async (analysisId: string) => {
    ...
    try {
      ...
      if (data.status === 'optimization_completed' || data.status === 'optimization_failed' || data.status === 'initial_failed') {
        // Once completed, fetch full analysis data once if configured to do so
        if (defaultConfig.fetchFullDataOnComplete && !state.hasFetchedFullData) {
