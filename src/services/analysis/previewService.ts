...
export class PreviewService {
  ...
  static async getPreview(analysisId: string): Promise<PreviewResponse['data']> {
    try {
      const response = await analysisApiClient.get<PreviewResponse>(`/analyze/${analysisId}/preview`);
  ...
  static async generatePDF(analysisId: string): Promise<{ success: boolean; file_url: string }> {
    try {
      const response = await analysisApiClient.post<{ success: boolean; file_url: string }>(`/analyze/${analysisId}/generate-pdf`);
